...
func TestGetRoute(t *testing.T) {
	r := newTestRouter()
	n, ps := r.getRoute("GET", "/hello/geekjc")
	...
	if ps["name"] != "geekjc" {
		t.Fatal("name should be equal to 'geekjc'")
	}
...
func TestGetRoute2(t *testing.T) {
	...
	ok2 := pattern == "/assets/*filepath" && ps2["filepath"] == "css/test.css"
	if !ok2 {
		t.Fatal("pattern shoule be /assets/*filepath & filepath should be css/test.css")
	}
}
